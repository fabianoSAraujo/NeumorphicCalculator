...
for (const opName of ["add", "subtract", "multiply", "divide", "percent"]) {
  document.querySelector(`.op__key[data-op=${opName}]`).onclick = opCallback(
    opName
  );
}
...
document.querySelector(".op__key[data-op=clear]").onclick = () => {
  $input.value = 0;
  buffer.length = 0;
};
document.querySelector(".op__key[data-op=negate]").onclick = () =>
